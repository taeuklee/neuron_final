...
<body>
	<div style="height: 100%; width: 100%;">
		...
		<form id="insertProjectForm" action="inviteMember.do?projectNo=${project.projectNo }" method="get" onsubmit="false" style="height: 100%; width: 50%; float: left;">
			<div style="float: left; width: 100%; height: 100%; overflow: auto" id="projectMember">
			
			</div>
			<input type="button" class="btn btn-primary" id="invite" style="position: relative; left:470px;" value="확인" onclick="insertTaskMember()">
		</form>
	...
	function insertProjectSearchMemberList() {
		...
		$.ajax({
			url : "insertTaskProjectSearchMemberList.do?empNo=${loginEmployee.empNo}&projectNo=${project.projectNo}",
			type : "get",
			data : {
			...
			success : function(data) {
				...
				if(data.length > 0){
					for(var i in data){
						...
						$div2.append($div3)
						$imgArea = $div3.append("<img src='/resources/euploadFiles/"+data[i].empFileReName+"' class='img-fluid rounded-start' style='height:100%;width:100%' alt='이미지영역'>")
